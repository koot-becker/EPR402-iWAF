...
def split_dataset(dataset, training_ratio=10):
    # Load the data
    data_path = f'Datasets/Original/{dataset}.csv'
...
def test_anomaly_classifier(dataset_name='ctf'):
    data_path = f'Datasets/GoBuster/{dataset_name}_training.csv'
    with open(data_path, 'r') as file:
        reader = csv.DictReader(file)
        training_data = [row for row in reader]

    data_path = f'Datasets/GoBuster/{dataset_name}_testing.csv'
    with open(data_path, 'r') as file:
        reader = csv.DictReader(file)
